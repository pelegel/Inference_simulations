...
def save_results(i, promot_count, prompt, response, response_tokens, duration, queue_time, conversation_context_length, total_context_length):
    ...
    with open(f"/ex1/outputs/user_{i}.txt", "a") as f:             # Generated response and statistics
    ...
    with open(f"/ex1/plots/generation_speeds.txt", "a") as f:         # Response generation speed
        f.write(f"{str(response_tokens/duration)}\n")
        
    with open(f"/ex1/plots/durations.txt", "a") as f:                 # Response generation duration
        f.write(f"{duration}\n")

    with open(f"/ex1/plots/queue_times.txt", "a") as f:               # Request's time in queue
        f.write(f"{queue_time}\n")

    with open(f"/ex1/plots/total_context_length.txt", "a") as f:      # Context length after current response
...
def plot_requests_status(running, waiting, total_context_length):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("/ex1/plots/Requests_Status_vs_Context_Length.png", dpi=300, bbox_inches="tight")
...
def plot_queue_times(queue_times):
    ...
    plt.grid(True)
    plt.savefig("/ex1/plots/Requests_Queue_Time.png", dpi=300, bbox_inches="tight")
    plt.clf()  # Clear the current figure
...
def plot_generation_speed(generation_speeds, total_context_length, queue_times):
    ...
    # Save and clear
    plt.savefig("/ex1/plots/Generation_Speed_vs_Context_Length.png", dpi=300, bbox_inches="tight")
    plt.clf()
...
if __name__ == "__main__":
    ...
    with open("ex1/prompts.json", 'r', encoding='utf-8') as file:
